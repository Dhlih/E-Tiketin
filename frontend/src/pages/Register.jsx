...
const Register = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/users/register`,
        {
          name,
          email,
          password,
  ...
  return (
    <div className="w-full h-screen flex items-center justify-center bg-[#F8F7F7] px-[3rem]">
      <div className="shadow-lg rounded-xl px-6 py-7 max-w-[370px] text-center bg-white outline-1 outline-[#D8DBE0]">
        ...
        <p className="mb-[1rem] text-sm mt-1">
          Sudah punya akun?
          <Link to="/login" className="text-[#007CFF] ml-1">
            Login aja
          </Link>
        </p>

        <div className="input-group">
          <div className=" rounded-md px-4 py-2 flex items-center space-x-[1rem] outline-[#D8D8D8] outline-1 text-sm">
            <FaRegUser size={15} className="opacity-50" />
            <input
              type="text"
              placeholder="nama"
